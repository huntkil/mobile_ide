...
main() {
    ...
    # 3. VNC 서버 상태 확인
    log_info "VNC 서버 상태 확인 중..."
    if ps aux | grep -E "(Xvnc|vncserver)" | grep -v grep &>/dev/null; then
        log_success "VNC 서버 실행 중 확인"
    else
        log_error "VNC 서버가 실행되지 않았습니다."
        return 1
    fi
    ...
    echo "  재시작: ./run_cursor_with_vnc.sh"
    echo ""
