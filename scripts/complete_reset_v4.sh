...
# Galaxy Android용 Cursor AI IDE 완전 초기화 스크립트 v4.0
# Author: Mobile IDE Team
# Version: 4.0.0
# Description: npm 오류 완전 해결, 모든 환경 완전 초기화, 안정적인 재설치
# Usage: ./scripts/complete_reset_v4.sh
...
show_version() {
    ...
    echo "설명: npm 오류 완전 해결, 안정적인 완전 초기화 및 재설치"
}
...
run_perfect_install() {
    ...
    # 2. 완벽 설치 실행
    log_info "termux_perfect_setup_v4.sh 실행 중..."
    if ./scripts/termux_perfect_setup_v4.sh; then
...
show_completion_summary() {
    ...
    echo "  - npm 오류가 발생하면 --skip-npm 옵션을 사용하세요"
    echo ""
}
...
main() {
    echo "=========================================="
    echo "  Galaxy Android용 Cursor AI IDE 완전 초기화 v4.0"
    echo "=========================================="
    echo ""
    ...
    if [ "$skip_npm" = true ]; then
        log_info "npm 없이 완벽 설치 실행 중..."
        run_perfect_install --skip-npm || {
            log_error "완벽 설치 실패"
            exit 1
        }
    else
