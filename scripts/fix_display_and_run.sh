...
echo "디스플레이 및 시스템 오류 해결 중..."

echo "1단계: VNC 서버 상태 확인 및 재시작..."
...
echo ""
echo "3단계: Ubuntu 환경에서 Cursor AI 실행..."
...
echo 'X11 연결 테스트 (Ubuntu 내부):'
xset q 2>/dev/null && echo 'X11 연결 성공' || echo 'X11 연결 실패'

echo 'Cursor AI 실행 (향상된 옵션)...'

# 더 안전한 옵션으로 실행
nohup ./squashfs-root/AppRun \
  --no-sandbox \
  --disable-gpu \
  ...
  --ignore-gpu-blocklist > /tmp/cursor_full.log 2>&1 &
...
if kill -0 \$CURSOR_PID 2>/dev/null; then
    ...
    echo '로그 확인:'
    tail -10 /tmp/cursor_full.log 2>/dev/null || echo '로그 파일 없음'
fi

echo '백그라운드에서 계속 실행 중...'
"
...
echo "2. 다른 VNC 앱 시도 (bVNC 등)"
echo "3. Ubuntu 로그 확인: proot-distro login ubuntu, tail -f /tmp/cursor_full.log"
echo ""
echo "디스플레이 및 시스템 오류 해결 완료!" 