...
# Galaxy Android용 Cursor AI IDE 완벽 복구 스크립트 (Termux 최적화)
...
# 전역 변수 (Termux 안전 버전)
SCRIPT_DIR="$(pwd)"
LOG_FILE="/tmp/cursor_restore_$(date +%Y%m%d_%H%M%S).log"
...
reinstall_cursor_ai() {
    ...
    # 임시 파일 정리
    rm -f /tmp/cursor*.AppImage 2>/dev/null || true
    rm -f /tmp/cursor*.deb 2>/dev/null || true
    ...
    for url in "${download_urls[@]}"; do
        ...
        if wget --timeout=60 --tries=3 -O /tmp/cursor.AppImage "$url" 2>/dev/null; then
            download_success=true
            break
        fi
        
        if curl --connect-timeout 60 --retry 3 -L -o /tmp/cursor.AppImage "$url" 2>/dev/null; then
            download_success=true
            break
        fi
    ...
    if [ "$download_success" = false ]; then
        ...
        echo "3. 다운로드한 파일을 /tmp/cursor.AppImage로 복사"
        echo ""
...
# AppImage 파일 복사
cp /tmp/cursor.AppImage ./cursor.AppImage
...
show_completion_message() {
    ...
    echo "  완전 재설치: ./termux_perfect_setup.sh"
    echo ""
...
main() {
    echo ""
    echo "🔧 Galaxy Android용 Cursor AI IDE 완벽 복구 스크립트 (Termux 최적화)"
    echo "=================================================================="
    echo ""
    
    # 로그 파일 초기화
    echo "복구 시작: $(date)" > "$LOG_FILE"
